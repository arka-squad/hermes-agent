...
# Override for tests and packagers; empty = the platform default below.
CORTEX_APP_PATHS: tuple = ()


def _cortex_app_paths() -> tuple:
    """Where the Cortex application lives once installed: the macOS bundle, or the
    per-user NSIS install on Windows (``productName: Cortex``). Never a caller path."""
    if CORTEX_APP_PATHS:
        return tuple(Path(p) for p in CORTEX_APP_PATHS)
    if sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local) if local else Path.home() / "AppData" / "Local"
        return (base / "Programs" / "Cortex" / "Cortex.exe",)
...
def _create_operation_without_plugin(home: Path, profile_label: str) -> dict:
    ...
    getuid = getattr(os, "getuid", None)  # POSIX ownership check; Windows has per-user %LOCALAPPDATA%
    if directory.is_symlink() or (getuid is not None and directory.stat().st_uid != getuid()):
