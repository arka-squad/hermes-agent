...
export function CortexProviderPanel() {
  ...
  const refresh = useCallback(async () => {
    try {
      setStatus(await getCortexStatus())
    } catch (err) {
  ...
  const activate = useCallback(async () => {
    ...
    try {
      const started = await startCortexActivation()
    ...
    timer.current = setInterval(() => {
      void (async () => {
        try {
          const next = await getCortexActivation(operationId)
